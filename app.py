...
# --- B. 詳情視圖 (進入觀念溯源時) ---
    ...
    with col_content:
        ...
        # 顯示分科解題撇步
        ext.exam_tips(curr_key, subject_label)

    with col_viz:
        # 局部觀念相依圖
        st.markdown("<p style='text-align:center; font-weight:600; font-size:0.8rem;'>觀念相依路徑</p>", unsafe_allow_html=True)
        ext.render_local_graph(curr_key, CONCEPTS)
        
        # 筆記匯出功能
        st.write("<br>", unsafe_allow_html=True)
        ext.export_study_notes(st.session_state.history, curr_key, CONCEPTS, subject_label)
    ...
    if parents:
        st.subheader("🔍 基礎不穩？追溯更底層的觀念：")
        cols = st.columns(len(parents))
        for i, p_key in enumerate(parents):
            p_data = CONCEPTS.get(p_key)
            if p_data:
                with cols[i]:
                    st.markdown(f"""
                    <div style="background:rgba(37, 99, 235, 0.05); padding:10px; border-radius:8px; border:1px solid rgba(37, 99, 235, 0.2); min-height:80px;">
                        <small style="color:#2563EB; font-weight:bold;">LV {p_data['level']}</small><br>
                        <b>{p_data['emoji']} {p_data['title']}</b>
                    </div>
        ...
        st.success("🏁 **已抵達學科基石。** 這已經是該領域最底層的定義或公理，掌握它就能打通所有上層建築。")
        st.balloons()
    ...
    st.write("<br>", unsafe_allow_html=True)
    if st.button("⬅️ 返回上一層觀念", use_container_width=False):
